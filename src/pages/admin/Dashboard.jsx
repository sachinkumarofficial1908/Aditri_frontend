...
import {
  Users, Package, ShoppingBag, MessageSquare, TrendingUp,
  Folder, AlertCircle, CheckCircle, Clock, LogOut,
  ...
  return (
    <aside className="w-64 bg-gray-900 text-white flex flex-col min-h-screen fixed left-0 top-0 z-40">
        ...
        {ADMIN_NAV.map(({ to, icon: Icon, label }) => (
          <Link key={to} to={to}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
              active === label.toLowerCase() ? 'bg-primary-600 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'
            }`}>
            <Icon size={18} />
            {label}
          </Link>
      ...
      <div className="p-4 border-t border-gray-800 space-y-2">
        <Link to="/" className="flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-gray-400 hover:bg-gray-800 hover:text-white transition-all">
...
function StatCard({ icon: Icon, label, value, color, sub }) {
  return (
    <motion.div whileHover={{ y: -3 }} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
...
export default function Dashboard() {
  ...
  if (isLoading) return (
    <div className="ml-64 p-8 animate-pulse">
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
        {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-36 bg-gray-200 rounded-2xl" />)}
      </div>
    </div>
  );
    ...
    <div className="flex min-h-screen">
      ...
      <main className="ml-64 flex-1 p-8 bg-gray-50">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
          ...
          <StatCard icon={MessageSquare} label="New Inquiries" value={s.newInquiries || 0} color="bg-red-500" sub="Awaiting response" />
          <StatCard icon={Folder} label="Projects" value={s.projects || 0} color="bg-teal-500" />
          <StatCard icon={TrendingUp} label="Revenue" value={`₹${((s.totalRevenue || 0) / 100000).toFixed(1)}L`} color="bg-green-500" sub="Paid orders" />
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="divide-y divide-gray-50">
              ...
              {data?.recentOrders?.map((order) => (
                <div key={order._id} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors">
                  <div>
                    <p className="text-sm font-medium text-gray-900">{order.orderId}</p>
                    <p className="text-xs text-gray-500">{order.user?.name || order.guestInfo?.name || 'Guest'}</p>
                  </div>
